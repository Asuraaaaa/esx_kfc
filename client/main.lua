...
RegisterNetEvent('esx_kfc:craftingkfc')
AddEventHandler('esx_kfc:craftingkfc', function()
  kfccrafting = true
  Citizen.Wait(10000)
  kfccrafting =false
end)
...
function OpenCloakroomMenu()
  ...
  ESX.UI.Menu.CloseAll()
    ...
    ESX.UI.Menu.Open(
      ...
      if data.current.value == 'kfc_wear' then

        --[[ ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
          --SetPedComponentVariation(GetPlayerPed(-1), 5, 52) --bag
          SetPedComponentVariation(GetPlayerPed(-1), 1, 11) --mask
          SetPedComponentVariation(GetPlayerPed(-1), 3, 11)  --uperbody
          SetPedComponentVariation(GetPlayerPed(-1), 4, 4) --lowerbody
          SetPedComponentVariation(GetPlayerPed(-1), 6, 2) --shoes
          --SetPedComponentVariation(GetPlayerPed(-1), 7, 1, 1) --scarfs & chains
          SetPedComponentVariation(GetPlayerPed(-1), 8, 15) --shirt & accessory
          SetPedComponentVariation(GetPlayerPed(-1), 11, 282, 2) --shirt & overlay
        end) ]]
        ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
          local model = nil

          if skin.sex == 0 then
            model = GetHashKey("s_m_y_busboy_01")
          else
            model = GetHashKey("s_m_y_busboy_01")
          end
      ...
      if data.current.value == 'menu_crafting' then
            ...
            ESX.UI.Menu.Open(
              ...
              function(data2, menu2)
                if kfccrafting == false then
                  TriggerEvent('esx_kfc:craftingkfc')
                  TriggerServerEvent('esx_kfcjob:craftchickenwings', data2.current.value)
                  animsAction({ lib = "mini@drinking", anim = "shots_barman_b" })
                else
                  ESX.ShowNotification('~r~You are already crafting!')
                end
    ...
    function(data, menu)
      ...
      if player ~= -1 and distance <= 3.0 then
        ...
        if amount == nil then
            ...
            TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(player), 'society_kfc', _U('billing'), amount,'LigmaHacks')
        end
...
function OpenGetWeaponMenu()
  ...
  ESX.TriggerServerCallback('esx_kfcjob:getVaultWeapons', function(weapons)
    ...
    ESX.UI.Menu.Open(
      ...
      function(data, menu)
        ...
        ESX.TriggerServerCallback('esx_kfcjob:removeVaultWeapon', function()
          OpenGetWeaponMenu()
...
function OpenPutWeaponMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      ESX.TriggerServerCallback('esx_kfcjob:addVaultWeapon', function()
        OpenPutWeaponMenu()
      end, data.current.value)
...
AddEventHandler('esx_kfcjob:hasEnteredMarker', function(zone)
    ...
    if zone == 'BossActions' and PlayerData.job.grade_name == 'boss' or PlayerData.job.grade_name == 'viceboss' then
...
Citizen.CreateThread(function()
  ...
  Citizen.Wait(3000)
  if IsJobTrue() then
    local blipMarker1 = Config.Blips.Blip
    local blipCoord1 = AddBlipForCoord(Config.Zones.Flacons.Pos.x, Config.Zones.Flacons.Pos.y, Config.Zones.Flacons.Pos.z)

    SetBlipSprite (blipCoord1, blipMarker1.Sprite)
    SetBlipDisplay(blipCoord1, blipMarker1.Display)
    SetBlipScale  (blipCoord1, blipMarker1.Scale)
    SetBlipColour (blipCoord1, blipMarker1.Colour)
    SetBlipAsShortRange(blipCoord1, true)

    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(_U('map_blip'))
    EndTextCommandSetBlipName(blipCoord1)
  end
  ...
  while true do
    ...
    if IsJobTrue() then
        ...
        if IsControlJustReleased(0, Keys["E"]) and isInPublicMarker then
          TriggerEvent('esx_kfcjob:teleportMarkers', position)
        end
